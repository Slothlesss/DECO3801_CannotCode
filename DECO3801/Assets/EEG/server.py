...
class CortexClient:
    def __init__(self):
        ...
        self.latest_relax = 0
        self.latest_interest = 0
        self.latest_frustration = 0

        #TCP socket
        self.tcp_client = None
        self.last_tcp_send_time = 0
    ...
    def frustration_level(self):
        ...
        if len(self.buffer) >= self.window_size and current_time - self.last_frustration_time >= 1:
            ...
            self.latest_frustration = frustration
    ...
    def start_tcp_server(self):
        ...
        print("Waiting for Unity to connect via TCP...")

        self.tcp_client, addr = server_socket.accept()
    ...
    def send_levels_to_unity(self):
        ...
        if self.tcp_client and current_time - self.last_tcp_send_time >= 1:
            self.last_tcp_send_time = current_time

            relax = getattr(self, "latest_relax", 0)
            interest = getattr(self, "latest_interest", 0)
            frustration = getattr(self, "latest_frustration", 0)
        
            fatigue = self.fatigue_level(relax) or 0
            focus = self.focus_level(interest) or 0

            print(f"relax={relax:.2f}, interest={interest:.2f}, "
              f"frustration={frustration}, fatigue={fatigue}, focus={focus}")
            ...
            try:
                self.tcp_client.sendall((json.dumps(message) + "\n").encode("utf-8"))
